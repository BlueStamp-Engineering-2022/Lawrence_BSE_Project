...
const int POT_ARDUINO_PIN[N_POTS] = {A0, A1, A2, A3}; //* pins of each pot connected directly to the Arduino
...
const int TIMEOUT = 30; //* amount of time the potentiometer will be read after exceeding the varThreshold
int varThreshold = 5; //* threshold for the variation in the potentiometer signal
unsigned long PTime[N_POTS] = {0}; // previously stored time
unsigned long timer[N_POTS] = {0}; // stores the time that has passed since the timer was reset

///////////////////////////////////////////
// midi
byte midiCh; //* MIDI channel to be used
byte note = 36; //* lowest grade to use
byte cc; //* Lowest MIDI CC to use
...
void potentiometers() {
  ...
  */
  ...
  for (int i = 0; i < N_POTS; i++) { // Loop all pots
    ...
    potCState[i] = analogRead(i); //try replacing POT_ARDUINO_PIN[i] with just "i"
    ...
    if (timer[i] < TIMEOUT) { // If the timer is less than the maximum time allowed, it means the potentiometer is still moving
      if(analogRead(i) == analogRead(A0)) {
              pot0();
              //Serial.println(analogRead(i));
        }
      
      if(analogRead(i) == analogRead(A1)) {
              pot1();
              //Serial.println(analogRead(i));
        }

      if(analogRead(i) == analogRead(A2)) {
              pot2();
              //Serial.println(analogRead(i));
        }

       if(analogRead(i) == analogRead(A3)) {
              pot3();
              //Serial.println(analogRead(i));
        }
...
void pot0() {
      controlChange(midiCh, 1, midiCState[0]); // (channel, CC number, CC value)
      MidiUSB.flush();
      if (midiPState[0] != midiCState[0]) {
        potPState[0] = potCState[0]; // Store current potentiometer reading to compare with next
        midiPState[0] = midiCState[0];
      }
}

void pot1() {
      controlChange(midiCh, 64, midiCState[1]); // (channel, CC number, CC value)
      MidiUSB.flush();
      if (midiPState[1] != midiCState[1]) {
        potPState[1] = potCState[1]; // Store current potentiometer reading to compare with next
        midiPState[1] = midiCState[1];
      }
}

void pot2() {
      controlChange(midiCh, 7, midiCState[2]); // (channel, CC number, CC value)
      MidiUSB.flush();
      if (midiPState[2] != midiCState[2]) {
        potPState[2] = potCState[2]; // Store current potentiometer reading to compare with next
        midiPState[2] = midiCState[2];
      }
}
...
void pot3() {
      ...
      else if(analogRead(3) > 750){
        note = 60;
      }
      else if(analogRead(3) > 500){
        note = 48;
      }
      else if(analogRead(3) > 250){
        note = 36;
      }
      else{
        note = 24;
      }
      if (midiPState[3] != midiCState[3]) {
        potPState[3] = potCState[3]; // Store current potentiometer reading to compare with next
        midiPState[3] = midiCState[3];
      }
}
